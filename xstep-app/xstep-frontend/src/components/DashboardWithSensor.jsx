...
const DashboardWithSensor = ({ riskScore, riskLevel, setRiskLevel, setRiskScore }) => {
  ...
  useEffect(() => {
    if (hasRecentData && sensorData.pressure_percentage !== undefined) {
      // Calculate risk score based on pressure readings
      // Higher sustained pressure = higher risk
      const pressureRisk = Math.min(100, sensorData.pressure_percentage * 1.2)
      
      // Update risk score more frequently when foot press is active
      if (footPressActive) {
        const newScore = Math.max(riskScore, pressureRisk)
  ...
  }, [sensorData, hasRecentData, footPressActive, pressurePercentage, riskScore, setRiskLevel, setRiskScore])

  // Simulate other data updates (battery, sync time)
  useEffect(() => {
  ...
  // Update connection status based on sensor
  useEffect(() => {
    setIsConnected(sensorConnected && hasRecentData)
  }, [sensorConnected, hasRecentData])
    ...
    <div className="p-4 max-w-md mx-auto">
      ...
      >
        ...
        >
          ...
          <span className="text-sm font-medium">Live Sensor</span>
        </Button>
      </motion.div>

      {/* Daily Stats - Enhanced with real sensor data */}
      <motion.div 
        ...
        <div className="grid grid-cols-3 gap-4 text-center">
          <div>
            <div className="text-2xl font-bold text-blue-600">
              {hasRecentData ? Math.round(pressurePercentage) : '--'}
            </div>
          ...
          <div>
            ...
            <div className="text-xs text-slate-600">Sensor Status</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-orange-600">
              {footPressActive ? 'YES' : 'NO'}
            </div>
            <div className="text-xs text-slate-600">Foot Press</div>
          </div>
        </div>
      </motion.div>
    </div>
...
export default DashboardWithSensor
