...
const FootMapWithSensor = () => {
  ...
  // Simulate foot data with real sensor input
  ...
  // Update foot data based on real sensor readings
  useEffect(() => {
    if (hasRecentData && sensorData.pressure_percentage !== undefined) {
      // Map sensor pressure to different foot zones
      // For demo purposes, we'll primarily affect the heel area
      const sensorPressure = Math.min(100, sensorData.pressure_percentage)
      
      setLeftFootData(prev => ({
        ...prev,
        heel: Math.max(0, Math.min(100, sensorPressure + (Math.random() - 0.5) * 10)),
        arch: Math.max(0, Math.min(100, sensorPressure * 0.3 + (Math.random() - 0.5) * 8)),
        ball: Math.max(0, Math.min(100, sensorPressure * 0.8 + (Math.random() - 0.5) * 12)),
        bigToe: Math.max(0, Math.min(100, sensorPressure * 0.4 + (Math.random() - 0.5) * 8)),
        toes: Math.max(0, Math.min(100, sensorPressure * 0.3 + (Math.random() - 0.5) * 6)),
        temperature: 36.2 + (footPressActive ? 0.5 : 0) + (Math.random() - 0.5) * 0.2
      }))
      
      // Right foot gets some correlated but different readings
      setRightFootData(prev => ({
        ...prev,
        heel: Math.max(0, Math.min(100, sensorPressure * 0.9 + (Math.random() - 0.5) * 10)),
        arch: Math.max(0, Math.min(100, sensorPressure * 0.25 + (Math.random() - 0.5) * 8)),
        ball: Math.max(0, Math.min(100, sensorPressure * 0.7 + (Math.random() - 0.5) * 12)),
        bigToe: Math.max(0, Math.min(100, sensorPressure * 0.45 + (Math.random() - 0.5) * 8)),
        toes: Math.max(0, Math.min(100, sensorPressure * 0.35 + (Math.random() - 0.5) * 6)),
        temperature: 36.5 + (footPressActive ? 0.3 : 0) + (Math.random() - 0.5) * 0.2
      }))
    }
  }, [sensorData, hasRecentData, footPressActive])

  // Fallback to simulated data when no real sensor data
  useEffect(() => {
    if (!hasRecentData) {
      ...
      <div className="flex flex-col items-center">
        ...
        <div className="relative">
          ...
          <svg width="120" height="200" viewBox="0 0 120 200" className="drop-shadow-lg">
            ...
            {/* Pressure zones with enhanced animation for real sensor data */}
            {/* Heel */}
            <motion.circle
              cx="60"
              cy="160"
              ...
              transition={{ 
                duration: hasRecentData ? 0.2 : 0.5,
                ease: hasRecentData ? "easeOut" : "easeInOut"
              }}
            />
    ...
    <div className="p-4 max-w-md mx-auto">
      ...
      >
        ...
        <p className="text-slate-600">
          {hasRecentData ? 'Real-time sensor data' : 'Simulated data'} - pressure and temperature monitoring
        </p>
      </motion.div>
        ...
        >
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm font-medium text-green-800">
              Live sensor data - Pressure: {Math.round(pressurePercentage)}%
            </span>
          </div>
        </motion.div>
      )}
        ...
        {hasRecentData && (
          <div className="mt-3 pt-3 border-t border-slate-200">
            <div className="text-xs text-slate-600">
              Real-time data from ESP32 sensor • Last update: {sensorData.seconds_since_last_reading}s ago
            </div>
          </div>
        )}
      </motion.div>
...
export default FootMapWithSensor
